...
現在の alpha baseline artifact 系列 (`0.1.x`、先頭 artifact は `0.1.0`) の support boundary wording は [SUPPORT-BOUNDARY.md](SUPPORT-BOUNDARY.md) に固定する。README でも同じ文言をそのまま使う。
...
- [SUPPORT-BOUNDARY.md](SUPPORT-BOUNDARY.md): `v1alpha` / `0.1.x` の support boundary の正本
...
- `diag_render` は user-owned location / confidence / phase / semantic role を踏まえて lead group を決め、low-confidence case では raw compiler wording を主経路に戻しつつ omission を明示する
- `diag_render` の profile budget は `default` / `concise` / `verbose` / `ci` ごとに固定され、warning suppression・excerpt 数・template/macro/include 圧縮・child note 表示が deterministic に揃う
