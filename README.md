...
- `xtask`: `check`, `replay`, `snapshot`, `bench-smoke`, `fuzz-smoke`, `human-eval-kit`, `rc-gate`, `self-check`, `package`, `install`, `rollback`, `uninstall`, `vendor`, `hermetic-release-check`, `release-publish`, `release-promote`, `release-resolve`, `install-release`
