...
- [eval/rc/README.md](eval/rc/README.md): RC gate が ingest する metrics / issue budget / fuzz / UX sign-off evidence の置き場
...
- `cargo xtask rc-gate` は curated replay / rollout matrix / benchmark smoke / deterministic replay と manual RC evidence を 1 コマンドで集約し、`metrics-report.json`, `rc-gate-report.json`, `rc-gate-summary.md` を出力できる
...
- `xtask`: `check`, `replay`, `snapshot`, `bench-smoke`, `rc-gate`, `self-check`, `package`, `install`, `rollback`, `uninstall`, `vendor`, `hermetic-release-check`, `release-publish`, `release-promote`, `release-resolve`, `install-release`
...
cargo xtask rc-gate --report-dir target/rc-gate --metrics-manual-report eval/rc/metrics-manual-eval.json --allow-pending-manual-checks
