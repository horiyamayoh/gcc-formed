...
公開 support boundary は「GCC 15 が primary enhanced-render path」「GCC 13/14 は compatibility-only」と固定されており、ingestion spec も GCC 15+ を production quality path、GCC 13/14 を production rendering 対象外と定義している。[R2][R3]  
実装も `SupportTier::A/B/C` によって 15+ / 13–14 / それ未満を分け、Tier B の render は passthrough に落とし、capture runtime は `inject_sarif` による `-fdiagnostics-add-output=sarif:...` 注入を中心に組まれている。[R4][R5][R6]
...
README は現在の support boundary を  
「GCC 15 is the primary enhanced-render path」  
「GCC 13/14 are compatibility-only paths」  
と固定している。[R1]  
`SUPPORT-BOUNDARY.md` も同文言で、GCC 15 外では enhanced-render guarantees を境界外としている。[R2]

さらに ingestion spec は、GCC 15+ のみを production quality の wrapper rendering 対象、GCC 13–14 を production rendering 対象外、JSON を hot path core dependency にしない、と明記している。[R3]  
...
`diag_cli_front/src/mode.rs` は Tier B に対し、`Render` 要求時でも `tier_b_render_unsupported=passthrough` で passthrough を選び、notice でも GCC 13/14 を compatibility mode と明記している。[R5]  
...
GCC 13/14 は「Tier B だから render 不可」ではなく、  
