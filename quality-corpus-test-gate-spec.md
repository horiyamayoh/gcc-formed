...
### 11.6 Band B representative matrix

GCC13-14 を first-class beta band として扱う場合、curated corpus は version だけでなく `ProcessingPath` と fallback contract を明示できなければならない。
...
Band B fixture を gate に含めるときは、`VersionBand` を A/B だけで潰さず、`ProcessingPath` ごとの coverage を集計すること。
...
real compiler matrix は、GCC 15 で multiple output / SARIF-first の本命経路が成立し、GCC 13-14 では Band B の first-class beta coverage を `NativeTextCapture` と `SingleSinkStructured` の両方で明示する、という product 前提を test に写したものである。 [R1][R2]
...
| Legacy | GCC 12 latest patch or distro default | periodic / corpus ingest only |
...
| GCC 12- | MUST NOT render | MAY shadow(raw-only) | MUST | unsupported for enhanced path |

Band B の fixture / gate では、`GCC13-14` をまとめて扱うのではなく、`ProcessingPath` ごとに `bounded_render` と `honest_fallback` を区別すること。
