...
| Compatibility | GCC version / linker / env 差異に耐えること | matrix pass rate, fallback correctness | nightly / release |
...
GCC13-14 / GCC9-12 を first-class beta band として扱う場合、curated corpus は version だけでなく `ProcessingPath` と fallback contract を明示できなければならない。

| VersionBand | ProcessingPath | Corpus expectation | Preferred meta tags |
|---|---|---|---|
| `GCC13-14` | `NativeTextCapture` | bounded render か honest fallback のどちらを期待するかを fixture で明示する | `band:gcc13_14`, `processing_path:native_text_capture`, `fallback_contract:honest_fallback` |
| `GCC13-14` | `SingleSinkStructured` | explicit structured capture の bounded render を fixture で明示する | `band:gcc13_14`, `processing_path:single_sink_structured`, `fallback_contract:bounded_render` |
| `GCC9-12` | `NativeTextCapture` | useful-subset family では bounded render か honest fallback のどちらを期待するかを fixture で明示する | `band:gcc9_12`, `processing_path:native_text_capture`, `fallback_contract:bounded_render` |
| `GCC9-12` | `SingleSinkStructured` | explicit JSON structured capture の bounded render を fixture で明示する | `band:gcc9_12`, `processing_path:single_sink_structured`, `fallback_contract:bounded_render` |

Band B / Band C fixture を gate に含めるときは、`VersionBand` を A/B だけで潰さず、`ProcessingPath` ごとの coverage を集計すること。
...
- tags (`syntax`, `template`, `linker`, `ci-critical`, `band:gcc13_14`, `processing_path:native_text_capture`, `processing_path:single_sink_structured`, `fallback_contract:bounded_render`, `fallback_contract:honest_fallback`, etc.)
...
3. **Canonical analysis snapshot**
   - `ir.analysis.json`
4. **View model snapshot**
   - `view.<profile>.json`
5. **Rendered text snapshot**
   - `render.<profile>.txt`
...
| adapter bug | facts | SARIF mapping / residual parse |
| ranking change | analysis / view | lead group / first action rules |
...
real compiler matrix は、GCC 15 で multiple output / SARIF-first の本命経路が成立し、GCC 13-14 / GCC 9-12 では narrower beta coverage を `NativeTextCapture` と explicit `SingleSinkStructured` で band-aware に明示する、という product 前提を test に写したものである。 [R1][R2]

### 15.1 compiler version matrix

| 群 | 対象 | gate レベル |
|---|---|---|
| Primary | GCC 15 latest patch | PR / nightly / release |
| Compatibility | GCC 14 latest patch | nightly / release |
| Compatibility | GCC 13 latest patch | nightly / release |
| Legacy | GCC 12 latest patch or distro default | periodic / representative replay gate |

### 15.2 mode matrix

| Version | `render` | `shadow` | `passthrough` | 備考 |
|---|---|---|---|---|
| GCC 15+ | MUST | MUST | MUST | 本命経路 |
| GCC 14 | MUST on `NativeTextCapture`; explicit `SingleSinkStructured` is allowed | MUST(raw-only acceptable) | MUST | first-class beta / narrower preservation |
| GCC 13 | MUST on `NativeTextCapture`; explicit `SingleSinkStructured` is allowed | MUST(raw-only acceptable) | MUST | first-class beta / narrower preservation |
| GCC 12- | MUST on `NativeTextCapture` useful-subset representative fixtures; explicit `SingleSinkStructured` (JSON) is allowed and MUST be tagged separately | MAY shadow(raw-only) | MUST | experimental beta / narrower wins and narrower preservation |

Band B / Band C の fixture / gate では、`GCC13-14` / `GCC9-12` をまとめて扱うのではなく、`ProcessingPath` ごとに `bounded_render` と `honest_fallback` を区別すること。
...
5. **upstream compiler bug or incompatibility**
   - 製品側回避が必要か、support tier を下げるか判断
...
- GCC 15 Tier A E2E が 5 family 以上通る
...
- GCC 13/14 compatibility gate 安定
