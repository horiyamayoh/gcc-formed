...
real compiler matrix は、GCC 15 で multiple output / SARIF-first の本命経路が成立し、GCC 13 で SARIF 自体が導入されたという product 前提を test に写したものである。 [R1][R2]
...
| GCC 14 | SHOULD NOT default | MUST(raw-only acceptable) | MUST | compatibility |
| GCC 13 | SHOULD NOT default | MUST(raw-only acceptable) | MUST | compatibility |
