...
RepairUnit inference は `DiagnosticEvidenceGraph` の proof class を入力とする deterministic
must-link closure である。Proven structural edge だけが partition を縮約し、tentative relation、
ambiguous parent、弱い native-text evidence は separate visible unit のまま残す。独立 anchor の
衝突は integrity issue として fail-open し、legacy EpisodeGraph/score-first analysis は移行期間中の
shadow diff と compatibility consumer のためだけに side-by-side で保持する。
