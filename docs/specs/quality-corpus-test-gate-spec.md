...
family inventory anchor が GCC15 evidence だけ、または older-band representative replay cell の一部だけを持ち、older-band matrix coverage をまだ全面的には claim できない場合、その anchor は `matrix_applicability` を流用してはならない。代わりに `meta.yaml` に `older_band_applicability.shared_contract_when_emitted: true` と、`gcc13_14` / `gcc9_12` x `native_text_capture` / `single_sink_structured` 各 cell の `status` / `note` を記録しなければならない。未証明 cell は `missing_representative_evidence`、既に checked-in fixture で証明済みの cell は `representative_evidence` とし、後者では `representative_fixtures` を列挙して shared-contract proof と missing coverage を混同させてはならない。この inventory は applicability と evidence inventory の明示が目的であり、representative stop-ship matrix coverage そのものではない。`openmp`, `analyzer`, path/toolchain residue, あるいは older-band core C++ families のように emitted cell をまだ claim できない family は、`note` で explicit applicability gap として扱い、silent omission にしてはならない。

C の core/parser/fallback older-band pack は path-anchored evidence として扱う。現行 anchor は `c/preprocessor_directive/case-01` (`gcc13_14/single_sink_structured` preprocessor), `c/syntax/case-11` (`gcc9_12/single_sink_structured` parser/fix-it), `c/syntax/case-12` / `c/type/case-11` / `c/linker/case-11` (`native_text_capture` parser/type/linker), `c/macro_include/case-13` (`gcc13_14/single_sink_structured` include/name path), `c/macro_include/case-01` (honest fallback), `c/partial/case-07` (partial disclosure) である。sister path がその family をまだ claim していない場合、coverage report や human-eval packet で暗黙に補完してはならず、representative anchor では `matrix_applicability.note`、non-representative family inventory では `older_band_applicability` の path-scoped note で applicability を明示すること。

C++ の older-band proof pack は `cpp/overload/case-07` と `cpp/template/case-13` / `cpp/template/case-14` / `cpp/template/case-15` を shared-contract representative anchor として扱う。`access_control`, `deleted_function`, `deprecated`, `enum_switch`, `exception_handling`, `inheritance_virtual`, `init_order`, `move_semantics`, `pointer_reference`, `scope_declaration` のような GCC15-only core family は、matching older-band representative replay cell が入るまで `older_band_applicability` で unclaimed cell を明示しなければならない。`overload` / `template` が older-band `single_sink_structured` evidence を持っていても、`native_text_capture` の未証明 cell を暗黙に補完してはならず、`older_band_applicability` で path-scoped gap を明示すること。

modern C++ applicability pack は `cpp/constexpr/case-01`, `cpp/lambda_closure/case-01`, `cpp/lifetime_dangling/case-01`, `cpp/structured_binding/case-01`, `cpp/designated_init/case-01`, `cpp/three_way_comparison/case-01`, `cpp/concepts_constraints/case-01`, `cpp/coroutine/case-01`, `cpp/module_import/case-01`, `cpp/ranges_views/case-01` を `gcc13_14` representative anchor として扱い、same fixture root に checked-in `gcc15/dual_sink_structured` companion snapshot を保持して shared-contract proof を補強する。これらの anchor は `gcc9_12` matrix coverage を claim しているわけではないため、`gcc9_12` x `native_text_capture` / `single_sink_structured` の各 cell は `older_band_applicability` で明示しなければならない。`three_way_comparison`, `concepts_constraints`, `coroutine`, `module_import`, `ranges_views` のように GCC9-12 内でも front-end availability が version-sensitive な family は、per-cell note でその制約を明示し、newer-band evidence から band-wide emitted support を推論させてはならない。`module_import` のように nearest emitted proof が native-text residual になる anchor でも、family/headline/first-action の shared contract を保持しつつ stronger older-band claim は fabricate しないこと。
...
- non-representative な older-band inventory fixture では `older_band_applicability.shared_contract_when_emitted`
- non-representative な older-band inventory fixture で未証明 cell を残すときの `older_band_applicability.<version_band>.<processing_path>.status` / `.note`
- family inventory anchor が既存 older-band proof を指すときの `older_band_applicability.<version_band>.<processing_path>.representative_fixtures`
- representative anchor が companion snapshot を同一 fixture 配下に持つときでも、未証明 older-band cell は `older_band_applicability` で明示し、companion snapshot の存在だけで matrix coverage を claim してはならない
...
| GCC 13 | MUST on `NativeTextCapture`; explicit `SingleSinkStructured` is allowed | MUST | MUST | shared in-scope contract; representative `gcc13_14` lane, including modern C++ anchor fixtures with checked-in GCC15 companion proof |
...
sealed report の `candidate_sha` は `qualification_candidate_sha` の authority である。
RC / stable gate はその SHA と signed payload の `payload_source_sha` を
`release-commit-chain-v1` で比較する。runtime、fixture、canonical snapshot、repair
oracle、corpus metadata、qualification validator、gate semantics、packaging logic の
いずれかが変わった場合、docs-only manifest で救済してはならず、新 candidate SHA
で qualification を再実行する。product/source gate 自体も payload source の isolated
worktree で実行し、その commit を `gate_source_sha` として provenance に残す。
