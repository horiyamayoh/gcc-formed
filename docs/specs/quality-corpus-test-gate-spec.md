...
C の core/parser/fallback older-band pack は path-anchored evidence として扱う。現行 anchor は `c/preprocessor_directive/case-01` (`gcc13_14/single_sink_structured` preprocessor), `c/syntax/case-11` (`gcc9_12/single_sink_structured` parser/fix-it), `c/syntax/case-12` / `c/type/case-11` / `c/linker/case-11` (`native_text_capture` parser/type/linker), `c/macro_include/case-13` (`gcc13_14/single_sink_structured` include/name path), `c/macro_include/case-01` (honest fallback), `c/partial/case-07` (partial disclosure) である。sister path がその family をまだ claim していない場合、coverage report や human-eval packet で暗黙に補完してはならず、fixture metadata の `matrix_applicability.note` または同等の path-scoped note で applicability を明示すること。
