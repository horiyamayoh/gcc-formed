...
non-representative な C semantics / systems / toolchain fixture が GCC15 evidence だけを持ち、older-band representative replay cell をまだ持たない場合、その fixture は `matrix_applicability` を流用してはならない。代わりに `meta.yaml` に `older_band_applicability.shared_contract_when_emitted: true` と、未証明の `gcc13_14` / `gcc9_12` x `native_text_capture` / `single_sink_structured` 各 cell の `status` / `note` を記録しなければならない。この inventory は applicability と unclaimed coverage の明示が目的であり、representative stop-ship matrix coverage を意味しない。`openmp`, `analyzer`, path/toolchain residue のように older-band emitted cell をまだ claim できない family は、`note` で explicit applicability gap として扱い、silent omission にしてはならない。
