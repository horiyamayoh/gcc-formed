...
- `cpp/init_order/case-01` のような init_order では、宣言順の evidence card を fully expanded に保ち、`when initialized here` は `default` / `concise` / `ci` で summary-only に落としてよい。これは初期化 site を隠すためではなく、宣言順の根拠を前面に残して lead を 1 件に保つためである。
...
1. first user-owned invocation site
