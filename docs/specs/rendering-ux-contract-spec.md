...
- **関連 ADR**:
  ...
  - `adr-initial-set/adr-0034-presentation-v2-subject-first-blocks.md`
...
| profile | 主用途 | session mode (failure runs) | target lines / lead block | hard max lines / lead block | source excerpts | template frames | macro/include frames | candidate notes | warnings on failure |
|---|---|---:|---:|---:|---:|---:|---:|---:|---|
| `default` | local TTY | `all_visible_blocks` | 18 | 28 | 2 | 5 | 4 | 3 | summarize |
| `concise` | pipe / quick scan | `all_visible_blocks` | 10 | 14 | 1 | 3 | 2 | 2 | suppress |
| `verbose` | investigation | `all_visible_blocks` | 40 | 80 | 6 | 20 | 12 | 10 | show |
| `ci` | CI log | `all_visible_blocks` | 12 | 16 | 1 | 3 | 2 | 2 | summarize |
| `debug` | internal triage | `all_visible_blocks` | 60 | 120 | 8 | 30 | 20 | 20 | show |
| `raw_fallback` | safe escape hatch | `raw_first` | n/a | n/a | 0 | 0 | 0 | 0 | raw-first |
...
2 つの visible root が同じ family と同じ canonical location を共有する場合、higher-severity root を先に出さなければならない。  
同一点の warning が failing error より前に並んではならない。
...
| `visible_block_count` | integer | block として表示した visible root 数 |
| `session_mode` | enum | `all_visible_blocks | lead_plus_summary | capped_blocks | raw_first` |
| `suppressed_group_count` | integer | collapsed notice または tail summary に圧縮した group 数 |
...
| `internal_family` | string or null | analysis / rulepack / public JSON 側の machine family |
| `display_family` | string | terminal header に出す human-facing family |
| `severity_label` | string | `error`, `warning`, `note` など |
| `title` | string | user が最初に読む subject |
| `canonical_location` | string or null | 最も見せるべき location |
| `location_host` | enum | `header_suffix | evidence_suffix | excerpt_header | dedicated_line | none` |
...
| `evidence_lines` | array<string> | family-specific evidence slot line |
...
| フィールド | 型 | 意味 |
|---|---|---|
| `location_header` | string | `@ path:line:col` または dedicated excerpt header 相当 |
...
visible root の ordering は lead selection で決めるが、Presentation V2 の session model では **ordering と block 化を切り離してはならない**。

- `default` / `concise` / `verbose` / `debug` / `ci` の subject-first preset では、visible root は並び順どおり block 化する。
- `document.document_analysis.episode_graph` がある場合、独立 root group は全件 visible に保たなければならない。follow-on / duplicate / dependent group は独立 root として数えず、visible parent block の collapsed notice または hidden count に吸収してよい。
- 低信頼 group、passthrough-heavy group、declaration/definition conflict などは ordering を変えうるが、visible root を summary-only へ降格させる理由にはしない。
- `cpp/init_order/case-01` のように強く関連する evidence root が複数ある場合でも、「1 visible root = 1 block」の mental model は保ち、補助根拠は evidence / collapsed notice / tail section へ落とす。
...
### 11.4 tail summary / legacy summary の形式

summary-only は current-authority の built-in default では visible root 用の primary modelではない。summary-only に落としてよいのは次に限る。

- cascade-hidden member の omission notice
- warning-tail summary
- `lead_plus_summary` を明示した legacy preset / compatibility mode
...
omitted 3 dependent diagnostics from the same cascade
```

`debug` profile では suppressed member review のために、summary-only group の直下へ短い explainability block を追加してよい。

- `debug-facts:` は `group_ref`, `role`, `visibility_floor` を含み、必要なら `best_parent_group_ref` と `evidence_tags` を続ける
- `debug-policy:` は suppression / summary-only / omission notice を決めた wrapper policy を 1 行で説明する
- `debug-raw:` は raw provenance に辿るための `capture_refs` を出してよい
- 上記は `debug` 限定であり、`default` / `concise` / `ci` の出力量を増やしてはならない
- `evidence_tags` や policy line を compiler facts と混同させてはならない
...
8. `debug` profile では `group_analysis.best_parent_group_ref`, `group_analysis.evidence_tags`, `visibility_floor` を suppressed member explainability に使ってよいが、fact line と policy line は分けなければならない
...
## 12. header / subject / confidence 規則

### 12.1 canonical header grammar

Presentation V2 の canonical header grammar は次を基本とする。

```text
<severity>: [<display-family>] <subject>[ @ <location-suffix>]
```

- `severity` は human-facing severity label である
- `display-family` は terminal presentation 専用の family label である
- `subject` は user が最初に読む短い主題である
- `location-suffix` は interactive subject-first preset では inline を基本とする

### 12.2 display family と internal family

- `internal_family` は analysis / rulepack / public JSON の machine semantics である
- `display_family` は terminal header に出す human-facing family label である
- `display_family` は `internal_family` より粗くてもよく、preset / config によって解決されてよい
- public JSON は `internal_family` と semantic headline を維持し、`display_family` や template id を public contract に昇格しない
...
2. family-specific synthesized subject
3. lead node の `message.raw_text`
...
|---:|---|
| `>= 0.85` | subject-first header を無修飾で使用してよい |
| `0.60 - 0.84` | subject-first header を維持しつつ、evidence / action の表現を弱めてよい |
| `0.35 - 0.59` | generic subject または raw message を header に使い、`why:` は honesty line としてのみ使ってよい |
...
- `why:` は high-confidence core family の canonical header/evidence grammar には含めず、generic / fallback / low-confidence honesty に限定する。

### 12.6 raw message 保持

lead node の raw compiler message が subject と実質同値でない場合、以下のいずれかを満たすべきである。

1. verbose/debug で raw message を明示表示する
2. footer / secondary line から raw message に辿れる
3. low-confidence / generic block で `why:` line として露出する
...
expanded block の section 順は以下で固定する。
...
| action | `help` |
| contrast slot | `want`, `got`, `via` |
| lookup / ownership slot | `name`, `use`, `need`, `from`, `near` |
| conflict / linker slot | `now`, `prev`, `symbol`, `archive` |
| chain intro | `while`, `from`, `through` |
| raw fallback | `raw` |
| omission | `omitted` |

`why` と `because` は generic / low-confidence / fallback honesty のために残してよいが、high-confidence family の core slot catalog には置かない。

### 13.3 first action の位置

`first_action` がある場合、**header の直後** または **primary excerpt の直前** に置かなければならない。候補が複数ある場合、最も局所的で低コストなものを選ぶ。

### 13.4 header line の最小要件

interactive subject-first preset の header line には少なくとも以下を含む。

- severity
- display family
- subject

`ci` profile の first line は preset-dependent とし、次のいずれでもよい。

```text
path:line:column: error: <subject>
```

```text
error: [<display-family>] <subject>
```

interactive default の contract を CI first-line policy に従属させてはならない。

### 13.5 canonical location の最小要件

location がある場合、expanded block は location を header 付近または excerpt header 付近に必ず出す。

- `default` / `concise` / `verbose` / `debug` の subject-first preset では inline suffix を優先する
- header が長すぎる場合、location は evidence suffix または excerpt header に移してよい
- dedicated `--> path:line:col` line は verbose/debug、legacy preset、または inline placement が不誠実になる場合に限って使ってよい
- `ci` では path-first と subject-first のどちらでもよいが、preset ごとに 1 つの契約へ固定しなければならない
...
### 15.2 canonical slot catalog

renderer は family ごとに次の slot catalog から必要な行を選んでよい。
...
### 15.5 `why:` / `because:` の位置づけ

renderer は family ごとに短い causal wording を使ってよいが、`why:` は Presentation V2 の high-confidence core grammar ではない。

- `why:` は `generic_block`, low-confidence honesty, raw-preservation disclosure に使ってよい
- `because:` は slot catalog に載らない補助因果 line として使ってよい
- family-specific slot が十分に取れる場合、`why:` / `because:` でそれを置き換えてはならない
- compiler facts と矛盾してはならない

例:

- `because: the selected overload expects 2 arguments`
- `why: original compiler wording is preserved below because confidence is low`
...
1. subject-first header
2. `want:` / `near:` slot
3. failure point の primary excerpt
4. 直前の likely local cause（不足した `;`, `)`, `}` など）

**MUST NOT**: parser recovery が生んだ cascade errors をすべて expanded にすること。

### 17.2 type mismatch / conversion / overload

この family では、**expected vs actual** を subject-first block の直後へ出す。
...
2. `want:` expected
3. `got:` actual
4. `via:` declaration site または selected overload
5. call / assignment site excerpt
6. remaining candidate notes の summary
...
```text
omitted 9 other overload candidates
```
...
- subject-first header
- `want:` / `got:` または equivalent evidence が取れるなら先に出す
- failing point を 1 件表示
- first user-owned frontier を lead context として 1 件表示
- outermost user-owned caller を 1 件表示
- 同一 frontier 上の candidate / deduction repeat は 1 件の representative に畳んでよい
...
1. subject-first header
2. `from:` first user-owned invocation site
3. `through:` macro definition site または直近 expansion source
...
1. subject-first header
2. `symbol:` primary symbol（demangled 名があればそれを優先）
3. `from:` 最も役立つ object/archive context
4. role（undefined reference, multiple definition など）
5. source 由来 reference site があれば 1 件まで

`collect2_summary` は driver が付ける supporting linker context であり、独立した linker root ではない。具体的な linker root（たとえば `linker.undefined_reference`）がある場合、`collect2_summary` が lead を奪ってはならず、root を補完する summary-only / context note に留める。
...
CI first line は preset-dependent であり、interactive default の contract を拘束してはならない。

location がある group の first line は以下のいずれかを preset ごとに固定してよい。
...
```text
error: [raw] showing original compiler diagnostics because structured rendering is incomplete
...
| `presentation_preset` | built-in default | subject-first / legacy などの presentation preset |
| `presentation_file` | none | external presentation policy file |
| `session_mode` | profile dependent | `all_visible_blocks | lead_plus_summary | capped_blocks` |
| `warning_visibility` | auto | warning 抑制 |
| `path_policy` | shortest_unambiguous | path 表示戦略 |
| `location_policy` | inline_preferred | header suffix / excerpt header / dedicated line の優先順位 |
| `display_family_catalog` | built-in | internal family -> display family の解決 |
| `label_catalog` | built-in | `help` / `want` / `got` などの表示ラベル |
...
### 23.2 presentation config の責務

presentation policy が決めてよいのは、少なくとも次の範囲である。
...
- per-profile first-line policy

presentation policy は次を決めてはならない。

- public JSON schema
- internal family semantics
- cascade relation facts
- compiler-owned raw facts の削除

### 23.3 presentation config の fail-open

presentation customization は non-fatal とする。

- preset / TOML parse 失敗 -> built-in default preset
- unknown template / family mapping -> `generic_block` または該当 line skip
- invalid location policy -> built-in default
- invalid label override -> built-in label

presentation config が壊れていても compile/link invocation 全体を止めてはならない。

### 23.4 rollout

Presentation V2 の rollout は次の順で進める。
...
```text
error: [syntax] expected ';' after this declaration @ src/main.c:12:5
help: add ';' after 'int x = 10'
want: ';'
near: the next declaration starts before this one closes
12 |     int x = 10
   |     ^^^^^^^^^^ expected ';' here
...
```text
error: [type_mismatch] arguments do not match @ src/parse.cc:48:20
help: convert the argument to int or call the overload that accepts text
want: int
got : const char *
via : void set_limit(int) @ include/config.hpp:19:6
48 |     set_limit("42");
...
```text
error: [type_mismatch] constructor arguments do not satisfy this instantiation @ src/build.cc:77:14
help: pass an allocator argument or use the default-constructible specialization
want: default-constructible T or allocator-aware constructor
got : T = std::pair<int, std::string>
via : make_widget<T>
...
through: include/io_macros.h:14:9 expands to '(read_count(fd))'
...
```text
src/main.cc:1:1: error: [linker] symbol could not be resolved
  symbol: foo::Bar::baz()
  from  : main.o
  omitted 3 additional references to the same symbol
```

### 26.6 multiple visible roots (`default`)
...
error: [missing_name] identifier is not declared @ src/main.cpp:8:5
...
### 26.7 low-confidence mixed fallback

```text
error: [raw] showing a conservative wrapper view @ third_party/lib.hpp:201:7
note: some compiler details were not fully structured
why: original compiler diagnostics are preserved below because confidence is low
raw:
