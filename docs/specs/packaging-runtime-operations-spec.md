...
plain `gcc-formed --version` は backend GCC の compiler introspection として passthrough する。wrapper 自身の identity 確認には `--formed-version` / `--formed-version=verbose` / `--formed-self-check` を使う。

### 15.2 `--formed-version=verbose` の最小出力

- attested release identity、または `unknown (not attested)`
- immutable payload product version / build commit / primary archive SHA-256（利用可能な場合）
- target triple
- git commit
- build profile
- rustc/cargo version
...
- install identity record と embedded payload identity の整合。欠落・改ざん・不一致時は stable を推測せず `not_attested` とする
...
- `--formed-self-check`（release/payload identity、backend GCC version、VersionBand、ProcessingPath を 1 コマンドで収集する）
- 必要に応じて human-readable な `--formed-version=verbose`
- backend compiler version
- target triple
- install channel / artifact URL / checksum
- release identity attestation source と payload commit/archive SHA-256
