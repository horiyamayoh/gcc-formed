...
> **Authority**
> This is the current Diagnostic IR contract. `v1alpha` in this document names the schema/version lineage, not the current product maturity line. The product maturity is governed by the current release policy (`v1.0.0 stable`; immutable payload `1.0.0-rc.1`).
...
#### Syntax recovery relation

Parse/preprocess diagnostic が同一の compiler-declared `TextEdit`（path、half-open/point span、
replacement を含む）を持つ場合、その edit は `SameFixitSpan / Proven` must-link となり、
grounded action と primary repair anchor を提供する。異なる non-overlapping edit は別 unit の
cannot-merge signal である。fix-it がない weak native-text evidence は recovery wordingや8-line
近接だけでは mergeせず、`unresolved / never_hidden` を維持する。

1、2、3 個の missing semicolon の count contract は、GCC が各 defect を実際に観測した場合に
それぞれ 1、2、3 visible unit である。GCC が最初の parse failure 後に emit しなかった後続 defect
