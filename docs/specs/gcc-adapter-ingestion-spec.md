...
- `GCC15` では `DualSinkStructured` が既定 capability path である
- `GCC13-14` では `NativeTextCapture` が既定 capability path であり、explicit structured path は `SingleSinkStructured`
- `GCC9-12` でも `NativeTextCapture` と explicit `SingleSinkStructured` (JSON) を path-aware に扱う
- `GCC16+` と `Unknown` は separate evidence が揃うまで `Passthrough` / `PassthroughOnly` とする
...
### 4.4 GCC 13–14 / GCC 9–12 も shared in-scope contract に含める

**GCC 13–14 と GCC 9–12 は v1.0.0 stable の shared in-scope contract に含める。**
default capability path は `NativeTextCapture` で、`SingleSinkStructured` は explicit opt-in path として扱う。  
ただし、same-run native+structured の保証や default structured source は band/capability ごとに異なってよく、**public contract parity と internal capture parity を混同してはならない。**
...
1. `GCC16+` や unknown compiler を含む GCC 全バージョンで同一品質の wrapper rendering を直ちに提供すること
...
|---|---|---|---|---|---|
| A | `GCC15` | `render` / `shadow` / `passthrough` | `-fdiagnostics-add-output=sarif:file=...` | same-run で保持可能 | shared in-scope contract; dual-sink default capability |
| B | `GCC13-14` | `render` / `shadow` / `passthrough` | native text or `-fdiagnostics-format=sarif-file` | path-dependent / same-run native+structured は保証しない | shared in-scope contract; native text default capability |
| C | `GCC9-12` | `render` / `shadow` / `passthrough` | `json-file` / `json-stderr` or none | path-dependent / raw 保持を優先 | shared in-scope contract; native text default and explicit JSON structured path |
| D | `GCC16+` / `Unknown` | `passthrough` | none | raw 保持を優先 | out-of-scope until separately evidenced |
...
GCC 15 は (1)(2)(3)。  
GCC 13–14 と GCC 9–12 も current parity contract の中で (1)(2)(3) を持つが、structured source と same-run raw preservation の細部は capability/profile ごとに異なる。  
GCC 16+ と unknown は現時点では原則 (2) の conservative passthrough を中心とする。
...
## 15. Path A: GCC 15 dual-sink structured default path
...
1. **GCC 15 は dual-sink default capability だが、public contract は `GCC 9-15` で共有する**
