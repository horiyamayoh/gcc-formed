...
- `diag_cli_front` と `xtask` の root entrypoint 分割は完了し、次の構造改善優先は `diag_enrich` / `diag_render` の rule-engine hardening に移っている
- `diag_enrich` は message text に依存したヒューリスティックが強く、`diag_render` の selection / fallback もまだ浅い
...
**進捗メモ（2026-04-09）**  
この work package は `main` に反映済み。`diag_cli_front/src/main.rs` は thin entrypoint（172 lines）まで縮小し、`args` / `config` / `mode` / `execute` / `self_check` / `trace_output` へ責務を分離した。次の structural priority は `M1-P1-3: enrich / select / render の rule engine 化`。
...
8. **PR-8**: `diag_enrich` modularization + rule hardening
