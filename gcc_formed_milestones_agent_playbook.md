...
- `xtask/src/main.rs` が 6231 lines と依然として集中度が高い
- `diag_cli_front` の root entrypoint 分割は完了したが、次の構造改善対象は `xtask` 側に残っている
...
**進捗メモ（2026-04-09）**  
この work package は `main` に反映済み。`diag_cli_front/src/main.rs` は thin entrypoint（172 lines）まで縮小し、`args` / `config` / `mode` / `execute` / `self_check` / `trace_output` へ責務を分離した。次の structural priority は `M1-P1-2: xtask modularization`。
