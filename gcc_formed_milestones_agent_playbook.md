...
- `v1beta` という成熟度ラベルと `0.2.0-beta.N` 系 artifact version の意味は整理済みで、RC metrics packet も `main` に反映済みだが、fuzz / adversarial hardening と human evaluation kit はまだ未完了である
- `xtask` の root shell 化は完了したが、`xtask/src/commands/release.rs`（2235 lines）と `xtask/src/commands/corpus.rs`（1833 lines）に command cluster が残っている
- `diag_cli_front` と `xtask` の root entrypoint 分割は完了し、`diag_enrich` / `diag_render` の deterministic hardening も `main` に反映済みである
- public beta release path、RC gate automation、RC metrics instrumentation は `main` に反映済みで、次の未完了 work package は `M2-P0-3: fuzz / adversarial hardening を入れる` である
...
**進捗メモ（2026-04-09）**  
この work package は `main` に反映済み。`diag_cli_front/src/main.rs` は thin entrypoint（172 lines）まで縮小し、`args` / `config` / `mode` / `execute` / `self_check` / `trace_output` へ責務を分離した。`diag_enrich` と `diag_render` の hardening も反映済みで、次の未完了 work package は `M1-P0-5: public beta release path を作る`。
...
**進捗メモ（2026-04-09）**
この work package は `main` に反映済み。`xtask/src/main.rs` は dispatch-only の 466 lines まで縮小し、release / install / vendor / rollback / release repository 操作は `xtask/src/commands/release.rs`、replay / snapshot / acceptance report は `xtask/src/commands/corpus.rs`、shared filesystem / process helper は `xtask/src/util/fs.rs` / `xtask/src/util/process.rs`、unit tests は `xtask/src/tests.rs` へ分離された。`diag_enrich` と `diag_render` の hardening も反映済みで、次の未完了 work package は `M1-P0-5: public beta release path を作る`。
...
**進捗メモ（2026-04-09）**  
この work package は `main` に反映済み。`cargo xtask rc-gate` が curated replay / rollout matrix / benchmark smoke / deterministic replay と manual issue budget / fuzz / UX sign-off evidence を 1 コマンドで集約し、`rc-gate-report.json` / `rc-gate-summary.md` / sub-report を `report_dir` に保存する。`cargo xtask bench-smoke` は target-only stub から実測 report へ昇格し、`--formed-self-check` も canonical rollout matrix を JSON に含めるようになった。manual `rc-gate` GitHub Actions workflow と `eval/rc/*.json` の evidence template も追加済みで、次の未完了 work package は `M2-P0-2: metrics instrumentation を入れる`。
...
**目的**  
TRC / TFAH / first-fix success のような user-facing 指標は、完全には自動化できない。  
そのため RC では manual but repeatable な評価パックが必要になる。

**実装指示**
1. representative tasks を family ごとに選ぶ  
2. raw GCC / gcc-formed を比較する手順書を作る  
3. expert review 用の判定票を作る  
4. release artifact とは別に review bundle を保存する

**受け入れ基準**
- reviewer が毎回同じ手順で評価できる
- RC sign-off の根拠が残る
...
**目的**  
GCC 13/14 compatibility mode は、「賢い render ができているように見せる」よりも「どこまで保証しないか」を正直に出すほうが重要。

**実装指示**
- compatibility mode banner / wording / fallback reason を corpus と snapshot で固定する
- primary path と compatibility path を混同しない wording を採用する
- `KNOWN-LIMITATIONS.md` と表示文言を一致させる

**受け入れ基準**
- GCC 13/14 path が false confidence を与えない
- issue report で support tier が判別できる
...
**目的**  
canary → beta → stable の channel promote を、再 build なし・exact version pin 前提で運用する。

**主に触るファイル**
- `.github/workflows/`
- `README.md`
- `RELEASE-NOTES.md`
- `packaging-runtime-operations-spec.md`（contract change があれば ADR も）

**実装指示**
1. stable cut の runbook を作る  
2. GitHub Release と immutable version repository を一体で扱う  
3. exact version pin, checksum pin, signature pin を stable story に入れる  
4. rollback drill の結果を artifact として残す

**受け入れ基準**
- 1 回の stable candidate を canary → beta → stable と昇格できる
- promote が rebuild なしであることを確認できる
- rollback は 1 symlink operation で成立する

---

#### M3-P0-2: support / incident / rollback runbook を揃える
**目的**  
stable では「壊れたらどうするか」が docs にないと出荷できない。

**追加候補ファイル**
- `SUPPORT.md`
- `docs/runbooks/rollback.md`
- `docs/runbooks/incident-triage.md`
- `docs/runbooks/trace-bundle-collection.md`

**実装指示**
1. support tier ごとの triage 手順を書く  
2. trace bundle の取り方・redaction・添付物を書く  
3. rollback / uninstall / reinstall の手順を書く  
4. bug report template から runbook へ導線を張る

**受け入れ基準**
- bug report を受けた maintainer が runbook だけで初動できる
- stable user が rollback 手順を docs から辿れる

---

#### M3-P0-3: governance freeze をかける
**目的**  
stable 後は change control が厳しくなる。  
その前に、CLI / config / IR schema / render wording の変更規律を明文化する。

**実装指示**
1. `ADR-0020-stability-promises` を stable 向けに補強する  
2. breaking / non-breaking / experimental の線引きを定義する  
3. 1.0.0 後 backlog と pre-1.0 must-have backlog を切り分ける

**受け入れ基準**
- stable 後に何が breaking か説明できる
- reviewer が contract drift を検知できる
...
- `ADR-0025`: RC gate automation and sign-off contract
