...
### 2.1 現在の基線
`gcc-formed` は、spec-first な **Accepted Baseline / `v1alpha`** の実装リポジトリである。  
...
- Releases が未公開
- `v1alpha` という成熟度ラベルと `0.1.0` 系 artifact version の意味が文書上で混線している
...
- `diag_render` は low-confidence lead 時の 2 件目 group 展開、partial document の mixed fallback raw sub-block、CI/default enhanced path の `<temp-object>` 正規化まで反映済みで、次の未完了 work package は `M1-P0-5: public beta release path を作る` である
