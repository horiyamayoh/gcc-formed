...
| B | 13.x–14.x | `render` on `NativeTextCapture`; explicit `SingleSinkStructured`; `shadow` / `passthrough` for conflicts | native text or `-fdiagnostics-format=sarif-file` | path-dependent / same-run native+structured not guaranteed | first-class beta |
...
GCC 15+ は (1)(2)(3)。  
GCC 13–14 は v1beta では first-class beta として (2)(3) を中心に扱う。  
GCC 12 以下は原則 (2) のみ。
