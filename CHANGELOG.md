...
- Reworked the PR/nightly/RC gate contract so representative replay, self-check, and snapshot drift now block on `gcc9_12`, `gcc13_14`, and `gcc15`, while release-only packaging, install, rollback, repository-publish, fuzz, and dependency checks remain isolated to the `gcc15` release lane without narrowing the diagnostic contract.
...
- Rekeyed lane-7 corpus/testkit/human-eval metadata around `version_band`, `processing_path`, and `support_level`, added legacy tier-letter read compatibility in `diag_testkit`, and fixed `xtask` so untagged `GCC9-12` fixtures now default to `NativeTextCapture` instead of being misclassified as `Passthrough`.
