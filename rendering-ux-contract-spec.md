...
ここで renderer 自身の failure が予見できる場合、早めに `raw_fallback` へ落としてよい。

### 9.2 stage 2: group construction

group は以下の順で決める。

1. `analysis.group_ref` がある場合、それで group 化する
2. 無い場合、top-level root ごとに独立 group とする
3. `passthrough` node は独立 group として扱う

adapter が root を merge しない原則は維持する。renderer も、根拠のない独自 merge を行ってはならない。
...
| `suppressed_group_count` | integer | summary のみの group 数 |
...
summary-only group は 1〜2 行に収める。
