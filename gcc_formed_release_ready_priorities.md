...
4. **診断品質のコア rule がまだ文字列ヒューリスティック寄りで、出荷直前の品質調整リスクを押し上げている**
...
- `diag_enrich` の family 分類はかなりの部分が `contains("template")` や `contains("macro")` などの文字列ヒューリスティックに依存している
...
この項目は **2026-04-09 時点で `main` に反映済み**です。  
`diag_cli_front/src/main.rs` は thin entrypoint（172 lines）へ縮小され、CLI parsing、config merge、mode selection、execution planning、trace/self-check が別 module に分離されました。`xtask` modularization も同日 `main` へ反映済みで、release-ready 観点の次の structural / quality priority は `diag_enrich` / `diag_render` の rule engine 化です。
