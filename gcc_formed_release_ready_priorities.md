...
4. **public beta release path、RC gate automation、RC metrics packet は 2026-04-09 時点で `main` に反映済みで、次の user-visible priority は fuzz / adversarial hardening と human evaluation kit の固定である**
...
`xtask/src/main.rs` は thin CLI shell（466 lines）へ縮小され、release/install/vendor/release-repo cluster は `xtask/src/commands/release.rs`、replay/snapshot/acceptance report は `xtask/src/commands/corpus.rs`、共有 helper は `xtask/src/util/`、tests は `xtask/src/tests.rs` に分離されました。その上で `main` には `xtask/src/commands/rc_gate.rs` が追加され、`cargo xtask rc-gate` / `bench-smoke` / `metrics-report.json` を含む machine-readable RC report が動くようになっています。release-ready 観点の次の user-visible priority は fuzz / adversarial hardening と human-eval evidence の継続観測です。
