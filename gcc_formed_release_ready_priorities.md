...
4. **変更コストの高い集中実装（特に `xtask` と CLI orchestration）が、出荷直前の変更リスクを押し上げている**
...
`diag_cli_front/src/main.rs` は thin entrypoint（172 lines）へ縮小され、CLI parsing、config merge、mode selection、execution planning、trace/self-check が別 module に分離されました。release-ready 観点の次の structural priority は `xtask` modularization です。
