...
4. **`diag_render` の lead selection / low-confidence / CI profile hardening は 2026-04-09 時点で `main` に反映済みで、次の user-visible priority は public beta release path と release gate の運用固定である**
...
この項目は **2026-04-09 時点で `main` に反映済み**です。  
`diag_cli_front/src/main.rs` は thin entrypoint（172 lines）へ縮小され、CLI parsing、config merge、mode selection、execution planning、trace/self-check が別 module に分離されました。`xtask` modularization、`diag_enrich` / `diag_render` hardening も `main` へ反映済みで、release-ready 観点の次の user-visible priority は public beta release path と release artifact publication です。
