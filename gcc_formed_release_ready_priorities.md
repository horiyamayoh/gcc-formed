...
4. **診断品質の残タスクが `diag_render` の lead selection / low-confidence / CI profile hardening に集中している**
...
現状では、基盤はできていますが、結果品質のコアはまだ強くありません。

## 現状から見える問題

- `diag_enrich` の structured rule input 化は進んだが、`diag_render` 側ではまだ「どの improved path を成立させ、どこで raw fallback に戻すか」の境界が shallow な箇所が残っている
- `diag_render` は passthrough / failed document / selected card が空のとき raw fallback に落ちるため、**改善 path の成立率**が release gate で十分に見えていない
- snapshot の整合性は追っているが、**「この出力が人間にとって better か」の gate**がまだ弱い
...
`xtask/src/main.rs` は thin CLI shell（466 lines）へ縮小され、release/install/vendor/release-repo cluster は `xtask/src/commands/release.rs`、replay/snapshot/acceptance report は `xtask/src/commands/corpus.rs`、共有 helper は `xtask/src/util/`、tests は `xtask/src/tests.rs` に分離されました。`diag_enrich` の structured rule hardening も `main` へ反映済みで、release-ready 観点の次の structural / quality priority は `diag_render` の lead selection / low-confidence / fallback hardening です。
...
この項目は **2026-04-09 時点で `main` に反映済み**です。  
`diag_cli_front/src/main.rs` は thin entrypoint（172 lines）へ縮小され、CLI parsing、config merge、mode selection、execution planning、trace/self-check が別 module に分離されました。`xtask` modularization と `diag_enrich` structured rule hardening も `main` へ反映済みで、release-ready 観点の次の structural / quality priority は `diag_render` の lead selection / low-confidence / CI profile hardening です。
...
`diag_enrich` 側の rule hardening は **2026-04-09 時点で `main` に反映済み**です。context chain / phase / semantic role / symbol context / ownership を優先する structured rule input と、specific ingress family の conservative preservation が入ったため、この項目の残差は主に `diag_render` の selection explainability / low-confidence 境界 / fallback 境界にあります。
