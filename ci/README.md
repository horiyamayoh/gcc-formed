...
plan files は step order, support tier classification, synthetic skip metadata, summary-only command preview を固定する。workflow YAML 側では実際の shell command を `ci/gate_step.py` 経由で実行し、path-aware replay gate は `ci/gate_replay_contract.py` が `replay-report.json` を classification artifact に変換し、summary は `ci/gate_summary.py` が生成する。
