...
- `nightly-gate` では replay / self-check / snapshot の matrix steps が `gcc:12` / `gcc:13` / `gcc:14` / `gcc:15` すべてで blocker として実行され、`reference_path_only` steps だけが `gcc15_plus` reference-path run 以外で `skipped_by_policy` になる。
