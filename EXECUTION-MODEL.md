...
### 2.4 GCC 15 は privileged path であって唯一の製品ではない

vNext の delivery system は、GCC 15+ / GCC 13–14 / GCC 9–12 の 3 経路を並列に扱えるよう設計する。  
ただし、同時に「全部同じ保証を今すぐ主張する」ことはしない。

正しく分けるべき概念は次の 4 つである。

- **VersionBand**: `GCC15+`, `GCC13-14`, `GCC9-12`
...
- **SupportLevel**: public にどの程度保証するか
...
- `gcc_formed_milestones_agent_playbook.md`
...
| `Band` | single select | GCC15+ / GCC13-14 / GCC9-12 / Cross-cutting |
| `Processing Path` | single select | DualSinkStructured / SingleSinkStructured / NativeTextCapture / Passthrough / Cross-path |
| `Support Level` | single select | Public / Beta / Experimental / Internal-only |
...
1. 本書を repo 正本として追加
2. `gcc_formed_milestones_agent_playbook.md` を legacy 扱いに変更
...
- `gcc_formed_milestones_agent_playbook.md` の vNext 正本としての役割
- 旧 `SupportTier` 中心の delivery vocabulary
