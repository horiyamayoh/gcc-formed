...
- For Band B/Band C representative matrix coverage, prefer explicit meta tags such as `band:gcc13_14` or `band:gcc9_12`, `processing_path:native_text_capture` or `processing_path:single_sink_structured`, `surface:default`, `surface:ci`, and `surface:debug`, and `fallback_contract:bounded_render` or `fallback_contract:honest_fallback`.
