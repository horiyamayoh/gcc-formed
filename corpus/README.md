...
- Non-representative C semantics / systems / toolchain fixtures that only have GCC15 evidence today MUST use `older_band_applicability` instead of `matrix_applicability` to record unclaimed `gcc13_14` / `gcc9_12` x `native_text_capture` / `single_sink_structured` cells. Set `shared_contract_when_emitted: true`, add a per-cell `status`, and keep each `note` explicit enough that reviewers cannot mistake inventory for representative stop-ship coverage.
