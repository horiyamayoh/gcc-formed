...
- Family inventory anchors that need to record older-band evidence or explicit applicability without becoming representative matrix coverage MUST use `older_band_applicability` instead of `matrix_applicability` for those older-band cells. Set `shared_contract_when_emitted: true`, use a per-cell `status` / `note`, and when a cell is already proven, list the checked-in `representative_fixtures` so reviewers can distinguish shared-contract proof from missing coverage.
...
- Treat the C core/parser/fallback older-band pack as path-anchored evidence rather than an implicit cross-path claim. Current anchors are `c/preprocessor_directive/case-01` (`gcc13_14/single_sink_structured` preprocessor), `c/syntax/case-11` (`gcc9_12/single_sink_structured` parser/fix-it), `c/syntax/case-12` / `c/type/case-11` / `c/linker/case-11` (`native_text_capture` parser/type/linker), `c/macro_include/case-13` (`gcc13_14/single_sink_structured` include/name path), `c/macro_include/case-01` (honest fallback), and `c/partial/case-07` (partial disclosure). When a sister path does not yet claim one of these families, record that scope in `matrix_applicability.note` for representative anchors or `older_band_applicability` for non-representative family fixtures instead of implying coverage across the whole band.
- The C++ older-band proof pack currently uses `cpp/overload/case-07` and `cpp/template/case-13` / `cpp/template/case-14` / `cpp/template/case-15` as shared-contract anchors. GCC15-only core families such as `access_control`, `deleted_function`, `deprecated`, `enum_switch`, `exception_handling`, `inheritance_virtual`, `init_order`, `move_semantics`, `pointer_reference`, and `scope_declaration` MUST record their unclaimed older-band cells in `older_band_applicability` instead of implying cross-band or cross-path coverage. When `overload` or `template` still lack `native_text_capture` evidence on an older band, record that gap in `older_band_applicability` rather than treating `single_sink_structured` proof as a cross-path claim.
- The modern C++ applicability pack uses `cpp/constexpr/case-01`, `cpp/lambda_closure/case-01`, `cpp/lifetime_dangling/case-01`, `cpp/structured_binding/case-01`, `cpp/designated_init/case-01`, `cpp/three_way_comparison/case-01`, `cpp/concepts_constraints/case-01`, `cpp/coroutine/case-01`, `cpp/module_import/case-01`, and `cpp/ranges_views/case-01` as `gcc13_14` representative anchors with checked-in `gcc15/dual_sink_structured` companion snapshots. Their `gcc9_12` cells remain explicit `older_band_applicability` inventory until a checked-in representative replay cell exists. Version-sensitive families such as `three_way_comparison`, `concepts_constraints`, `coroutine`, `module_import`, and `ranges_views` MUST keep their GCC9-12 front-end limitations in the per-cell note instead of implying band-wide support from the newer-band evidence.
...
- `repair-unit-exact-count/` holds the causal 1/2/3-defect pack. Every case carries failing and passing source, independent repair patches, `repair-oracle.toml`, canonical `causal-map.json`, and contributes to `repair-unit-coverage.json`. Causal assertions, not snapshots or family labels, are authoritative.
...
- The primary fixture layout stores goldens under `snapshots/<version_band>/<processing_path>/`. If a fixture keeps extra presentation-specific artifacts such as `subject_blocks_v2` or `subject_blocks_v1`, nest them under that path root, for example `snapshots/gcc15/dual_sink_structured/subject_blocks_v2/`.
...
- When a representative anchor also carries a companion snapshot for another band or path, keep that evidence under the same fixture root with an explicit `snapshots/<version_band>/<processing_path>/...` path. Do not encode companion proof through legacy root-level aliases or by omitting the `older_band_applicability` inventory for still-unclaimed cells.
